...
class ComidaView(ttk.Frame):
    def __init__(self, parent, controller):
        ...
        # Hacer que el canvas permita desplazarse con el teclado o trackpad
        ...
        self.canvas.bind_all("<Button-4>", lambda event: self.canvas.yview_scroll(-1, "units"))  # Scroll hacia arriba (trackpad)
        self.canvas.bind_all("<Button-5>", lambda event: self.canvas.yview_scroll(1, "units"))   # Scroll hacia abajo (trackpad)

        # Título
        label = ttk.Label(self.frame, text="Productos de Comida", font=("Helvetica", int(16 * factor)))
    ...
    def cargar_productos(self):
        ...
        try:
            with open("comida.json", "r", encoding="utf-8") as file:
    ...
    def agregar_al_carrito(self, producto):
        """Lógica para agregar un producto al carrito (por ahora, solo imprime)"""
        print(f"Producto agregado al carrito: {producto['nombreProducto']} - {producto['precio']}")